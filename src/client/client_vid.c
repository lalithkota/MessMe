...
int video_call_func1_close_flag=0,video_call_func2_close_flag=0;
...
void* call_start(void* dump){
  move(LINES-1,0);
  clrtoeol();
  if(is_caller) printw("INCOMING CALL");
  else printw("INCOMING CALL?");
  refresh();
  ...
  // read yes or no from user if he is not the caller.. i.e., he is the callee
  if(!is_caller){
    int key;
    while(true){
      key=getch();
...
void* video_call(void* dump){
  move(LINES-1,0);
  clrtoeol();
  if(is_caller) printw("INCOMING VIDEO CALL");
  else printw("INCOMING VIDEO CALL?");
  refresh();
  ...
  int* arg_dump[3]={&call_server_sd,&video_call_func1_close_flag,&video_call_func2_close_flag};
  pthread_t func1_id, func2_id;
  pthread_create(&func1_id, NULL, video_call_func1, (void*)arg_dump);
  pthread_create(&func2_id, NULL, video_call_func2, (void*)arg_dump);
    ...
    if(buff[0]=='-'){
      if(buff[1]=='m'){
        ...
        if(buff[2]=='i'){
          ...
          is_caller=false;
          pthread_t tid;
          pthread_create(&tid,NULL,video_call,NULL);
        }
        else{
          reader_busy=true;
