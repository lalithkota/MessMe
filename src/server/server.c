...
//bool video_call_thread2_close_flag = false;
//bool video_call_thread_close_flag = false;
...
void* call_thread_main(void* dump){
  ...
  // listen(call_server_sd,100);
  unsigned clientLen = sizeof(client);
  int client1 = accept(audio_server_sd,(struct sockaddr *)&client,&clientLen);
  int client2 = accept(audio_server_sd,(struct sockaddr *)&client,&clientLen);
  ...
  if(bu1[0]!=1 || bu2[0]!=1){
    ...
    close(client1);
    close(client2);
    close(audio_server_sd);
    return NULL;
  }
  printf("both acks are good\n");
  //else .. then everything is normal so ... continue
  bu1[0] = 1;
  write(client1,bu1,1);
  write(client2,bu1,1);

  int call_thread_close_flag = 0;
  int call_thread2_close_flag = 0;

  int* arg_dump[4];
  arg_dump[0]=&client1;
  arg_dump[1]=&client2;
  arg_dump[2]=&call_thread_close_flag;
  arg_dump[3]=&call_thread2_close_flag;
  ...
  printf("A new call started %d %d\n",client1,client2);

  while(!(call_thread_close_flag==1 || call_thread2_close_flag==1));
  // wait till either of them finish
  // if even one exited .. quit both
  pthread_cancel(func1_thread_id);
  pthread_cancel(func2_thread_id);
...
void* call_thread_func1(void* dump){
  ...
  int* call_thread_close_flag = arg_dump[2];
  int* call_thread2_close_flag = arg_dump[3];
...
void* call_thread_func2(void* dump){
  ...
  *call_thread2_close_flag = 1;
  return NULL;
}

void* video_call_func2(void*);
void* video_call_func1(void*);
void* video_call_main(void* dump){
  struct sockaddr_in client;
  unsigned clientLen = sizeof(client);
  int client1 = accept(video_server_sd,(struct sockaddr *)&client,&clientLen);
  int client2 = accept(video_server_sd,(struct sockaddr *)&client,&clientLen);
  ...
  int video_call_func1_close_flag=0;
  int video_call_func2_close_flag=0;

  int* arg_dump[4];
  arg_dump[0]=&client1;
  arg_dump[1]=&client2;
  arg_dump[2]=&video_call_func1_close_flag;
  arg_dump[3]=&video_call_func2_close_flag;

  pthread_t func1_thread_id, func2_thread_id;
  pthread_create(&func2_thread_id,NULL,video_call_func1,arg_dump);
  pthread_create(&func1_thread_id,NULL,video_call_func2,arg_dump);

  printf("A new video call started\n");

  while(!(video_call_func1_close_flag==1 || video_call_func2_close_flag==1));
  // wait till either of them finish
  // if even one exited .. quit both
  // pthread_cancel(func1_thread_id);
  // pthread_cancel(func2_thread_id);
  ...
  close(client1);
  close(client2);
  close(video_server_sd);
  printf("video call quit success\n");
  return NULL;
}

int get_1k_term_buffer(int sd, unsigned char* final_buff, int total_size){
	int i=0;
	unsigned char small_buff[1024];
...
int put_1k_term_buffer(int sd, unsigned char* final_buff, int total_size){
	int i=0;
	for(i=0;i<total_size/1024;i++){
		write(sd,&final_buff[i*1024],1024);
	}
	return total_size;
}
...
void* video_call_func1(void* dump){
  ...
  int* client1 = arg_dump[0];
  int* client2 = arg_dump[1];
  int* video_call_func1_close_flag = arg_dump[2];
  int* video_call_func2_close_flag = arg_dump[3];

  unsigned char buff[40960];
  while(true){
    if(read(*client2, buff, sizeof(buff))<=0){
      printf("video func1 read failed");
			break;
		}
    ...
    if(write(*client1, buff, sizeof(buff))<=0){
      printf("video func1 write failed\n");
      break;
    }
    if(*video_call_func2_close_flag==1){
      break;
...
void* video_call_func2(void* dump){
  ...
  unsigned char buff[40960];
  while(true){
    if(read(*client1, buff, sizeof(buff))<=0){
      printf("video func2 read failed\n");
      break;
    }
    if(*video_call_func1_close_flag==1){
      break;
    }
    if(write(*client2, buff, sizeof(buff))<=0){
      printf("video func2 write fail\n");
      break;
    }
    if(*video_call_func1_close_flag==1){
      break;
...
//   int client2 = arg_dump[1];
//
//   unsigned char image_size_buff[4];
//   unsigned char buff[20480];
//   while(true){
//     if(read(client2, image_size_buff, 4)<=0){
//       printf("Cannot read the size of image, first.");
// 			break;
// 		}
//     if(video_call_thread2_close_flag){
//       break;
//     }
//
// 		unsigned int image_length=0;//*((int*)image_size_buff);
//     for(int i=0;i<4;i++) image_length |= (((unsigned int)image_size_buff[i])<<(8*i));
//     printf("image size buff:%d\n",image_length);
//
//     if(read(client2, buff, image_length)<=0){
//       printf("video func1 buffer read failed.\n");
//       break;
//     }
//     if(video_call_thread2_close_flag){
//       break;
//     }
//
//     if(write(client1, image_size_buff, 4)<=0){
//       printf("video func1 write image_size write failed\n");
//       break;
//     }
//     if(video_call_thread2_close_flag){
//       break;
//     }
//
//     if(write(client1, buff, image_length)<=0){
//       printf("video func1 write failed\n");
//       break;
//     }
//     if(video_call_thread2_close_flag){
...
//     }
//   }
//   video_call_thread_close_flag = true;
//   return NULL;
// }
//
// void* video_call_func2(void* dump){
...
//   int count=0;
//   while(true){
//     if(read(client1, image_size_buff, 4)<=0){
//       printf("video func2 image_size_read failed\n");
//       break;
//     }
//     if(video_call_thread_close_flag){
//       break;
//     }
//
//     unsigned int image_length=0;//*((int*)image_size_buff);
...
//     printf("image size buff:%d\n",image_length);
//
//     if(read(client1, buff, image_length)<=0){
//       printf("video func2 buff_read failed\n");
//       break;
//     }
//     if(video_call_thread_close_flag){
//       break;
//     }
//
//     if(write(client2, image_size_buff, 4)<=0){
//       printf("video func2 image_size_buff write fail\n");
//       break;
//     }
//     if(video_call_thread_close_flag){
//       break;
//     }
//
//     if(write(client2, buff, image_length)<=0){
//       printf("video func2 buffer write fail\n");
//       break;
//     }
//     if(video_call_thread_close_flag){
//       break;
//     }
//   }
//   video_call_thread2_close_flag = true;
//   return NULL;
// }
    ...
    if(buff[0]=='-'){
      ...
      if(buff[1]=='s'){
        ...
        if(receiver_id==0){
          ...
          write(other_clients[my_id-1],names[receiver_id-1],strlen(names[receiver_id-1]));
          write(other_clients[my_id-1],", with uname: ",strlen(", with uname: "));
          write(other_clients[my_id-1],unames[receiver_id-1],strlen(unames[receiver_id-1]));
        }
        else{
      ...
      // }
        ...
        if(buff[2]=='i'){
          ...
          pthread_t video_call_main_tid;
          pthread_create(&video_call_main_tid,NULL,video_call_main,NULL);

          write(other_clients[receiver_id-1],"-vi ",strlen("-vi "));
...
int main(int argc,char** argv){
  ...
  // Initialise the chat server socket...
  unsigned clientLen;
  ...
  //initialise the audio call server socket...
  audio_server_sd = socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);
  server.sin_family=AF_INET;
  server.sin_addr.s_addr=inet_addr(serverAddressString);
  server.sin_port=htons(port_no + 1);
  bind(audio_server_sd,(struct sockaddr *)&server,sizeof(server));
  listen(audio_server_sd,100);

  //initialise the video call server socket...
  video_server_sd = socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);
  server.sin_family=AF_INET;
  server.sin_addr.s_addr=inet_addr(serverAddressString);
  server.sin_port=htons(port_no + 2);
  bind(video_server_sd,(struct sockaddr *)&server,sizeof(server));
  listen(video_server_sd,100);

  printf("Normal Chat Server Address : %s at %d\n",serverAddressString,port_no);
  printf("Audio Server Address : %s at %d\n",serverAddressString,port_no+1);
  printf("Video Server Address : %s at %d\n",serverAddressString,port_no+2);
