...
void* video_call_main(void* dump){
  ...
  struct sockaddr_in client_asserver;
  client_asserver.sin_family=AF_INET;
  client_asserver.sin_addr.s_addr=;
  client_asserver.sin_port=htons(port+1);
  int client1 = socket(AF_INET, SOCK_DGRAM,0);
  //int client2 = socket(AF_INET, SOCK_DGRAM,0);
  if(connect(client1,(struct sockaddr *)&client_asserver,sizeof(client_asserver))<0){
    close(client1);
    return NULL;
  }
  ...
  video_call_thread2_close_flag = false;
  video_call_thread_close_flag = false;
  ...
  while(!(video_call_thread_close_flag && video_call_thread2_close_flag));
  // wait till either of them finish
  // if even one exited .. quit both
...
int main(int argc,char** argv){
  ...
  // later implementation changed this type
  video_server_sd = socket(AF_INET,SOCK_DGRAM,0);
  server.sin_family=AF_INET;
  server.sin_addr.s_addr=inet_addr(serverAddressString);
